...
begin
 ...
 if op='state' then
  return jsonb_build_object('role',r,
   'people',coalesce((select jsonb_agg(to_jsonb(x)) from (
    select member.* from cc_private.people member where member.company_id=c and (r='owner' or member.user_id=uid or (member.crew_id is not null and cc_private.crew_access(c,member.crew_id) and r in ('supervisor','sub_lead')))
 ...
 if op in ('schedule','cancel_schedule','report','assign_crew') then
  if op in ('schedule','assign_crew') then
   ...
   if r='supervisor' and not exists(select 1 from cc_private.crew_sites where crew_id=cr.id and job_key=s.job_key) then raise exception 'Owner must assign this job to the crew first' using errcode='42501'; end if;
   ...
   if op='schedule' then
    ...
    insert into cc_private.schedule_people(schedule_id,user_id,name) select target,user_id,name from cc_private.people where company_id=c and crew_id=cr.id and active;
    return jsonb_build_object('id',target);
   end if;
  else
   ...
   for event in select o.*,a.site_id from public.cc_observations o join public.cc_assignments a on a.id=o.assignment_id where o.assignment_id=aid and o.observed_at>=now()-interval '30 days' order by o.observed_at,o.event_id loop
    if event.transition='enter' and open_id is null then open_id:=event.event_id; open_time:=event.observed_at;
     ...
     if event.observed_at>open_time and event.observed_at<=open_time+interval '24 hours' and not exists(select 1 from cc_private.cards where source_key=open_id::text||':'||event.event_id::text) then
      if exists(select 1 from public.cc_observations other_enter join public.cc_assignments other_a on other_a.id=other_enter.assignment_id where other_a.user_id=uid and other_a.id<>aid and other_enter.transition='enter' and other_enter.observed_at<event.observed_at and coalesce((select min(other_exit.observed_at) from public.cc_observations other_exit where other_exit.assignment_id=other_a.id and other_exit.transition='exit' and other_exit.observed_at>other_enter.observed_at),'infinity'::timestamptz)>open_time) or exists(select 1 from cc_private.cards where user_id=uid and status<>'void' and started_at<event.observed_at and coalesce(ended_at,'infinity'::timestamptz)>open_time) then conflicts:=conflicts+1;
      else
 ...
 if op in ('approve','request_changes','submit','void') and card.revision is distinct from (d->>'revision')::integer then raise exception 'Timecard changed. Refresh first.' using errcode='40001'; end if;
 before_card:=to_jsonb(card);
